		...
		if self:is_rect_x(step[1], step[2], step[3]) then
			if self:is_rect_y(y_direction) then
				...
				if self.reverse then
					x_target = i == 1 and self.steps[x_direction][#self.steps[x_direction]]
						or self.steps[x_direction][i - 1]
				else
					x_target = i == #self.steps[x_direction] and self.steps[x_direction][1]
						or self.steps[x_direction][i + 1]
				end
				break
			else
...
function Windex:up(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_xy("center", "up")
	end
end

function Windex:down(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_xy("center", "down")
	end
end

function Windex:center(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_x("center")
	end
end

function Windex:right(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_x("right")
	end
end

function Windex:upper_right(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_xy("right", "up")
	end
end

function Windex:lower_right(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_xy("right", "down")
	end
end

function Windex:left(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_x("left")
	end
end

function Windex:upper_left(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_xy("left", "up")
	end
end

function Windex:lower_left(should_reverse)
	return function()
		Windex:new(should_reverse):shuffle_xy("left", "down")
	end
end
